...
public sealed class MysqlAdapter : IProviderAdapter
{
        ...
        CancellationToken ct)
    {
        ...
        {
            var sql = $@"
CREATE FUNCTION UpdatePositionV2(eventId CHAR(36)) RETURNS BIGINT
READS SQL DATA
DETERMINISTIC
BEGIN
...
ON DUPLICATE KEY UPDATE Id = Id;";
        ...
        CancellationToken ct)
    {
        ...
        var query = dbContext.Database.SqlQuery<long>($"SELECT UpdatePositionV2({parameter})");
        ...
        CancellationToken ct)
    {
        ...
        // Therefore we have to create a positions table by ourself and create the next position in the same transaction.
        // Read comments from the following article: https://dev.to/kspeakman/event-storage-in-postgres-4dk2
        var query = dbContext.Database.SqlQuery<long>($"SELECT UpdatePositionsV2({parameter})");
